/*
本题与一般的树形差分不同，将一条路径分成两条链，分别做差分
差分打标记的方法也很特别，因为要记录经过这条路径上的不同的人（即不同的深度）的次数，最后不好累计，不能直接加加或减减多少
所以在打标记的点的地方通过vector把每次具体要加的人的标记记录下来，
统计时用的差分数组表示到目前点为止满足d[i] + w[i]的各个深度（由差分数组的下标表示）的数量，或满足w[i] - d[i]的d[x]-2d[z]的数量

差分打标记的区间保证了在这条路径上跑步人，能到这条路径上的所有点
*/
#include<iostream>
#include<cstdio>
#include<cstring>
