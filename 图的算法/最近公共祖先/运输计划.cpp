/*
仔细分析发现，答案满足单调性 因此可以二分答案ansans，然后O(m)O(m)验证。
于是我们要做的就是验证给出的一个答案是否合法。
如果所有路径中的最长路径mx≤ansmx≤ans则当前答案可行，并且如果两者差值mx−ansmx−ans能够通过把一条边变为0来消去则答案也可行。
显然，由于只能改一条边，则这条边一定是所有超出ansans的路径的一条公共边。
因此问题转化为寻找在那些超出ansans的路径中，是否存在一条公共边，并且其权值大于等于mx−ansmx−ans。后者很容易判定，关键在于前者。
如何验证公共边？
...
除此之外还有最重要的一点，由于一共只有M条路径，于是我们可以把M条路径的长度以及LCA求好，避免在judgejudge时反复求解——这是一个大到可怕的优化
